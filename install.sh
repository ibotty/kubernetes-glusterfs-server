...
ExecStart=/usr/bin/docker run --stop-signal SIGPWR --rm --privileged --ipc host --net host -v /sys/fs/cgroups:/sys/fs/cgroups:ro -v /dev/mapper:/dev/mapper -v /dev/\${GLUSTER_VG}:/dev/\${GLUSTER_VG} -v /etc/lvm:/etc/lvm -v /run/lvm:/run/lvm -v /var/lib/glusterd:/var/lib/glusterd -v /etc/glusterfs:/etc/glusterfs -v /run/systemd/journal/dev-log:/dev/log -v /var/log/glusterfs:/var/log/glusterfs -e GLUSTER_VG=\${GLUSTER_VG} -e GLUSTER_POOL=\${GLUSTER_POOL} --name ${NAME} ${IMAGE}
